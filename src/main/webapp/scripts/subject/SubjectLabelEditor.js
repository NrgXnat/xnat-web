		...
		if (!this.panel) {
			...
			this.panel.handleEnter = function() {
				...
				if (that.modifiedLabel === "") {
					...
					if (confirm("Modifying the label of a " + that.header + " will result in the moving of files on the file server within the " + XNAT.app.displayNames.singular.project.toLowerCase() + "'s storage space.  And it may result in the re-execution of project level anonymization if a project level anonymization script is configured. Are you sure you want to make this change?")) {
				    	if(showReason){
