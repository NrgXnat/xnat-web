    ...
    this.loadFields = function(){
        ...
        jq.get(serverRoot + "/REST/search/elements/"+elementName + "?showAll=true&showGenerated=false&format=json")
            .done(function (data) {
                ...
                jq(data.ResultSet.Result).each(function (index, obj){
                    listOfFields+="<TR><TD>" + obj.FIELD_ID + "<input type='hidden' id='element" + index +"' value='" + obj.ELEMENT_NAME + "'/><input type='hidden' id='field" + index +"' value='" + obj.FIELD_ID + "'/></TD><TD><input type='text' id='header" + index +"' value='" + obj.HEADER + "'/></TD><TD><input type='text' id='desc" + index +"' value='" + obj.DESC + "'/></TD><TD><input type='checkbox' id='searchable" + index +"' " + ((obj.SEARCHABLE==='true')?" CHECKED ":"") + " /></TD><TD NOWRAP><input type='button' onclick='manager.update(" + index +");' value='Update'/> " + ((obj.REQUIRES_VALUE==="true")?"<input type='button' value='Add Value' onclick='manager.addSqlQueryValue(" + index +");'/>":"") + "</TD></TR>";
                });
