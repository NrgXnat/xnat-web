				...
				var labelBox = document.getElementById("new_label");
				  ...
				  if(XNAT.app._label.selectedLabel==""){
				    ...
				    if(this.selector.uri==undefined){
					    ...
					    if(confirm("Modifying the " + this.selector.config.header + " of an imaging " + XNAT.app.displayNames.singular.imageSession.toLowerCase() + " will result in the moving of files on the file server within the " + XNAT.app.displayNames.singular.project.toLowerCase() + "'s storage space.  And it will result in the re-execution of project level anonymization if a project level anonymization script is configured. Are you sure you want to make this change?")){
					    	if(showReason){
