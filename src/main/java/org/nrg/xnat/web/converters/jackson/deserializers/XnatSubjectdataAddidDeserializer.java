package org.nrg.xnat.web.converters.jackson.deserializers;

import com.fasterxml.jackson.core.JsonParser;
import com.fasterxml.jackson.databind.DeserializationContext;
import lombok.extern.slf4j.Slf4j;
import org.nrg.xdat.om.XnatSubjectdataAddid;

import java.io.IOException;

@XnatDeserializer
@Slf4j
public class XnatSubjectdataAddidDeserializer<T extends XnatSubjectdataAddid> extends AbstractBaseElementDeserializer<T> {
    private static final long serialVersionUID = 3988025147706154674L;

    @SuppressWarnings({"unchecked", "unused"})
    public XnatSubjectdataAddidDeserializer() {
        this((Class<T>) XnatSubjectdataAddid.class);
    }

    protected XnatSubjectdataAddidDeserializer(final Class<T> clazz) {
        super(clazz);
    }

    protected void handleField(final T instance, final String field, final JsonParser parser, final DeserializationContext context) throws IOException {
        // TODO: Implement datatype-specific deserialization
        switch (field) {
            case "addid":
                instance.setAddid(parser.getText());
                break;
            case "name":
                instance.setName(parser.getText());
                break;
            case "xnatSubjectdataAddidId":
                instance.setXnatSubjectdataAddidId(parser.getIntValue());
                break;
            default:
                super.handleField(instance, field, parser, context);
        }
    }
}

