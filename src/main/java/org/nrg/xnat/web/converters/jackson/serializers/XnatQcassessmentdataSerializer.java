package org.nrg.xnat.web.converters.jackson.serializers;

import com.fasterxml.jackson.core.JsonGenerator;
import com.fasterxml.jackson.databind.SerializerProvider;
import lombok.extern.slf4j.Slf4j;
import org.nrg.xdat.om.XnatQcassessmentdata;

import java.io.IOException;

@XnatSerializer
@Slf4j
public class XnatQcassessmentdataSerializer<T extends XnatQcassessmentdata> extends XnatMrassessordataSerializer<T> {
    private static final long serialVersionUID = 2562973252018339227L;

    @SuppressWarnings({"unchecked", "unused"})
    public XnatQcassessmentdataSerializer() {
        this((Class<T>) XnatQcassessmentdata.class);
    }

    protected XnatQcassessmentdataSerializer(final Class<T> clazz) {
        super(clazz);
    }

    @Override
    protected void serializeImpl(final T instance, final JsonGenerator generator, final SerializerProvider provider) throws IOException {
        // TODO: Implement datatype-specific serialization
        // TODO: Write out the "scans_scan" property here: java.util.List
    	writeNonNullField(generator, "scans", instance.getScans_scan());
        writeNonBlankField(generator, "type", instance.getType());
        super.serializeImpl(instance, generator, provider);
    }
}

