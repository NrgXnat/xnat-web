package org.nrg.xnat.web.converters.jackson.serializers;

import com.fasterxml.jackson.core.JsonGenerator;
import com.fasterxml.jackson.databind.SerializerProvider;
import lombok.extern.slf4j.Slf4j;
import org.nrg.xdat.om.XnatRfsessiondata;

import java.io.IOException;

@XnatSerializer
@Slf4j
public class XnatRfsessiondataSerializer<T extends XnatRfsessiondata> extends XnatImagesessiondataSerializer<T> {
    private static final long serialVersionUID = -4223124858383028166L;

    @SuppressWarnings({"unchecked", "unused"})
    public XnatRfsessiondataSerializer() {
        this((Class<T>) XnatRfsessiondata.class);
    }

    protected XnatRfsessiondataSerializer(final Class<T> clazz) {
        super(clazz);
    }

    @Override
    protected void serializeImpl(final T instance, final JsonGenerator generator, final SerializerProvider provider) throws IOException {
        // No class-specific properties to serialize
        super.serializeImpl(instance, generator, provider);
    }
}

