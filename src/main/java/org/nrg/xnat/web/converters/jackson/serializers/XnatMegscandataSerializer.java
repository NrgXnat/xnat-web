package org.nrg.xnat.web.converters.jackson.serializers;

import com.fasterxml.jackson.core.JsonGenerator;
import com.fasterxml.jackson.databind.SerializerProvider;
import lombok.extern.slf4j.Slf4j;
import org.nrg.xdat.om.XnatMegscandata;

import java.io.IOException;

@XnatSerializer
@Slf4j
public class XnatMegscandataSerializer<T extends XnatMegscandata> extends XnatImagescandataSerializer<T> {
    private static final long serialVersionUID = -7339859924095340959L;

    @SuppressWarnings({"unchecked", "unused"})
    public XnatMegscandataSerializer() {
        this((Class<T>) XnatMegscandata.class);
    }

    protected XnatMegscandataSerializer(final Class<T> clazz) {
        super(clazz);
    }

    @Override
    protected void serializeImpl(final T instance, final JsonGenerator generator, final SerializerProvider provider) throws IOException {
        // No class-specific properties to serialize
        super.serializeImpl(instance, generator, provider);
    }
}

