	...
	public ArrayList<XnatExperimentdata> getXnatExperimentData(String experimentId, UserI user, ArrayList<XnatExperimentdata> assesseds2, String type2) {
		...
		if (experimentId != null) {
			for (String s : XftStringUtils.CommaDelimitedStringToArrayList(experimentId)) {
				XnatExperimentdata expt = XnatExperimentdata.getXnatExperimentdatasById(s, user, false);
...
//            }
	...
	public static String getQuery(ArrayList<XnatImagescandata> scans, ArrayList<XnatExperimentdata> assesseds,
		...
		if (!scans.isEmpty()) {
			...
			if (hasResourceIds) {
				...
				query.append(" AND (").append(
						xnatTemplateUtil.getResourceIdsWhereClause(resourceIds, "abst.xnat_abstractresource_id"))
						.append(")");
			}
		}
	...
	public boolean insertCatalog(XnatResourcecatalog catResource, Integer eventId, UserI user) throws Exception {
		...
		if (recons.size() > 0) {
			if (assessed == null) {
				// getResponse().setStatus(Status.CLIENT_ERROR_NOT_FOUND, "Invalid session
				// id.");
				return false;
			}
			...
			return _catalogService.insertResourceCatalog(user, UriParserUtils.getArchiveUri(sub), catResource,
					eventId) != null;
		} else if (proj != null) {
			return _catalogService.insertResourceCatalog(user, UriParserUtils.getArchiveUri(proj), catResource,
					eventId) != null;
		}
		return true;
	}
	...
	public void checkResourceIDs(final List<String> resourceIds) throws Exception {
		...
		if (Iterables.any(resourceIds, CONTAINS_QUOTE)) {
			throw new Exception(
					"Possible SQL Injection attempt. The \"'\" character is not allowed in resource labels: "
							+ StringUtils.join(Iterables.filter(resourceIds, CONTAINS_QUOTE), ", "));
		}
		if (Iterables.any(resourceIds, HACK_CHECK)) {
			throw new Exception("Possible SQL Injection attempt: "
					+ StringUtils.join(Iterables.filter(resourceIds, CONTAINS_QUOTE), ", "));
		}
	}
		...
		if (!recons.isEmpty()) {
			...
			if (!assesseds.isEmpty()) {
				...
				if (isInResource) {
					...
					query.append(STARTER_FIELDS);
					query.append(
							", 'assessors'::TEXT AS category, expt.id::TEXT AS cat_id, COALESCE(xes.singular,xmeexpt.element_name)::TEXT AS cat_desc");
					if (includeURI) {
						query.append(
								",'/experiments/' || xiad.imagesession_id || '/assessors/' || expt.id || '/in' || '/resources/' || abst.xnat_abstractresource_id AS resource_path");
					}
					query.append(
							" FROM img_assessor_in_resource map LEFT JOIN xnat_experimentdata expt ON map.xnat_imageassessordata_id=expt.id ");
					if (includeURI) {
						query.append(" LEFT JOIN xnat_imageassessordata xiad ON expt.id=xiad.id ");
					}
					query.append(
							" LEFT JOIN xdat_meta_element xmeexpt ON expt.extension=xmeexpt.xdat_meta_element_id LEFT JOIN xdat_element_security xes ON xmeexpt.element_name=xes.element_name LEFT JOIN xnat_abstractresource abst ON map.xnat_abstractresource_xnat_abstractresource_id=abst.xnat_abstractresource_id LEFT JOIN xdat_meta_element xme ON abst.extension=xme.xdat_meta_element_id WHERE map.xnat_imageassessordata_id IN ('");
					...
					if (includeURI) {
						query.append(
								",'/experiments/' || xiad.imagesession_id || '/assessors/' || expt.id || '/out' || '/resources/' || abst.xnat_abstractresource_id AS resource_path");
					}
					query.append(
							" FROM img_assessor_out_resource map LEFT JOIN xnat_experimentdata expt ON map.xnat_imageassessordata_id=expt.id ");
					if (includeURI) {
						query.append(" LEFT JOIN xnat_imageassessordata xiad ON expt.id=xiad.id ");
					}
					query.append(
			...
			if (includeURI) {
				query.append(
						",'/projects/' || sub.project || '/subjects/' || sub.id || '/resources/' || abst.xnat_abstractresource_id AS resource_path");
			}
			query.append(
					" FROM xnat_subjectdata_resource map LEFT JOIN xnat_subjectdata sub ON map.xnat_subjectdata_id=sub.id LEFT JOIN xnat_abstractresource abst ON map.xnat_abstractresource_xnat_abstractresource_id=abst.xnat_abstractresource_id LEFT JOIN xdat_meta_element xme ON abst.extension=xme.xdat_meta_element_id WHERE xnat_subjectdata_id='");
			...
			if (includeURI) {
				query.append(
						",'/projects/' || map.xnat_projectdata_id || '/resources/' || abst.xnat_abstractresource_id AS resource_path");
			}
			query.append(
					" FROM xnat_projectdata_resource map LEFT JOIN xnat_abstractresource abst ON map.xnat_abstractresource_xnat_abstractresource_id=abst.xnat_abstractresource_id LEFT JOIN xdat_meta_element xme ON abst.extension=xme.xdat_meta_element_id WHERE xnat_projectdata_id='");
