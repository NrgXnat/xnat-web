...
public class XNATCatalogTemplateUtil extends XnatTemplateUtil {
	...
	public List<String> setResourcesIds(String requestedResourceId, UserI user, final boolean allowAll) {
		...
		if (!requestedResources.isEmpty()) {
			...
			if (!requestedResourceIds.isEmpty()) {
				...
				if (resourceIds.size() < requestedResourceIds.size()) {
					...
					// throw new ClientException(Status.CLIENT_ERROR_FORBIDDEN, "The user " +  getUser().getUsername() + " requested " + requested + " resources by ID (i.e. not by qualified resource label) but was denied access to " +
					// (resourceIds.isEmpty() ? "all" : requestedResourceIds.size()) + " of them: " StringUtils.join(requestedResourceIds, ", "));
				}
		...
		try {
			...
			setCatalogs(loadCatalogs(_resourceIds, true, allowAll, user));
		} catch (Exception e) {
