...
public class XNATCatalogTemplateUtil extends XnatTemplateUtil {
	...
	public List<String> setResourcesIds(String requestedResourceId, UserI user, final boolean allowAll) {
		...
		final List<String> requestedResources = StringUtils.isNotBlank(requestedResourceId)
				? Arrays.asList(requestedResourceId.split("\\s*,\\s*"))
				: Collections.<String>emptyList();
		if (!requestedResources.isEmpty()) {
			// Separate numeric and non-numeric IDs to start. Non-numeric IDs get qualified
			// by project/experiment/etc later.
			_resourceIds.addAll(Lists.newArrayList(
					Iterables.filter(requestedResources, Predicates.not(Predicates.containsPattern("^\\d+$")))));
			final List<String> requestedResourceIds = Lists
					.newArrayList(Iterables.filter(requestedResources, Predicates.containsPattern("^\\d+$")));

			// Add all numeric IDs that are either permitted based on associated security ID
			// or that have no security ID (in construction)
			if (!requestedResourceIds.isEmpty()) {
				final List<String> resourceIds = Lists.transform(
						Lists.newArrayList(Iterables.filter(getTemplate().query(QUERY_FIND_RESOURCE_SECURE_OBJECTS,
								new MapSqlParameterSource("resourceIds", requestedResourceIds),
								RESOURCE_MAP_ROW_MAPPER), new PermittedResourcePredicate(user))),
						ResourceMap.FUNCTION_RESOURCE_MAP);
				if (resourceIds.size() < requestedResourceIds.size()) {
					...
					// throw new ClientException(Status.CLIENT_ERROR_FORBIDDEN, "The user " +
					// getUser().getUsername() + " requested " + requested + " resources by ID (i.e.
					// not by qualified resource label) but was denied access to " +
					// (resourceIds.isEmpty() ? "all" : requestedResourceIds.size()) + " of them: "
					// + StringUtils.join(requestedResourceIds, ", "));
				}
		...
		try {
			...
			log.error("An error occurred trying to load catalogs for user {} from the resource IDs: {}",
					user.getUsername(), _resourceIds, e);
		}
		return _resourceIds;
	}
	...
	private static class PermittedResourcePredicate implements Predicate<ResourceMap> {
		...
		public boolean apply(final ResourceMap resourceMap) {
			...
			try {
				...
				log.error("The user {} requested the resource with ID {} on the object {}/ID[{}] but was denied access",
						getUser().getUsername(), resourceMap.getResourceId(), xsiType, resourceMap.getSecurityId());
				return false;
			} catch (Exception e) {
				log.error(
						"An error occurred trying to check permissions for user {} on the object {}/ID[{}], denying by default",
						getUser().getUsername(), xsiType, resourceMap.getSecurityId(), e);
				return false;
			}
		}
	...
	@Nonnull
	protected List<String> _resourceIds = new ArrayList<>();
	@Nonnull
	...
	private static final String QUERY_FIND_RESOURCE_SECURE_OBJECTS = "SELECT "
			+ "  a.xnat_abstractresource_id AS resourceId, " + "  r.uri AS uri, " + "  xme.element_name AS xsiType, "
			+ "  e.id AS securityId, " + "  e.project AS projectId " + "FROM " + "  xnat_abstractresource a "
			...
			+ "  LEFT JOIN xdat_meta_element xme ON e.extension = xme.xdat_meta_element_id " + "WHERE "
			+ "  a.xnat_abstractresource_id::VARCHAR(64) IN (:resourceIds) AND " + "  e.id IS NOT NULL " + "UNION "
			+ "SELECT " + "  a.xnat_abstractresource_id AS resourceId, " + "  r.uri AS uri, "
			+ "  'xnat:subjectData' AS xsiType, " + "  s.id AS securityId, " + "  s.project AS projectId " + "FROM "
			...
			+ "  LEFT JOIN xnat_resource r ON a.xnat_abstractresource_id = r.xnat_abstractresource_id "
			+ "  LEFT JOIN xnat_subjectdata S ON res.xnat_subjectdata_id = S.id " + "WHERE "
			+ "  a.xnat_abstractresource_id::VARCHAR(64) IN (:resourceIds) " + "UNION " + "SELECT "
			+ "  a.xnat_abstractresource_id AS resourceId, " + "  r.uri AS uri, " + "  'xnat:projectData' AS xsiType, "
			+ "  p.id AS securityId, " + "  p.id AS projectId " + "FROM " + "  xnat_projectdata_resource res "
			+ "  LEFT JOIN xnat_abstractresource a ON res.xnat_abstractresource_xnat_abstractresource_id = a.xnat_abstractresource_id "
			+ "  LEFT JOIN xnat_resource r ON a.xnat_abstractresource_id = r.xnat_abstractresource_id "
			+ "  LEFT JOIN xnat_projectdata p ON res.xnat_projectdata_id = p.id " + "WHERE "
