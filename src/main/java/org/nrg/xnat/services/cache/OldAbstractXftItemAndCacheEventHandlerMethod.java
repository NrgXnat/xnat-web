...
@Accessors(prefix = "_")
@Slf4j
public abstract class OldAbstractXftItemAndCacheEventHandlerMethod extends AbstractXftItemEventHandlerMethod implements CacheEventListener<String, Object> {
    /**
     ...
     */
    protected OldAbstractXftItemAndCacheEventHandlerMethod(final CacheManager cacheManager, final XftItemEventCriteria first, final XftItemEventCriteria... criteria) {
     ...
     */
    protected OldAbstractXftItemAndCacheEventHandlerMethod(final CacheManager cacheManager, final CacheEventListener cacheEventListener, final XftItemEventCriteria first, final XftItemEventCriteria... criteria) {
        super(first, criteria);
        _cache = cacheManager.getCache(getCacheName());
        //noinspection rawtypes
        _cacheEventListener = ObjectUtils.defaultIfNull(cacheEventListener, new DefaultGenericCacheEntryListener());
    ...
    public void onEvent(CacheEvent<? extends String, ?> event) {
        log.info("Got a {} event for cache item {}", event.getType(), event.getKey());
    }
    ...
    protected long getLatestOfCreationAndUpdateTime(final String cacheId) {
        // CACHING: This is just to clear errors.
        return getEhCache().get(cacheId).hashCode();
    }

    protected org.ehcache.Cache getEhCache() {
        final Object nativeCache = getCache().getNativeCache();
        if (nativeCache instanceof org.ehcache.Cache) {
            return ((org.ehcache.Cache) nativeCache);
        }
    ...
    protected List<String> getEhCacheKeys() {
        // CACHING: This is junk
        //noinspection unchecked
        return (List<String>) StreamUtils.asStream(getEhCache().iterator()).map(entry -> ((org.ehcache.Cache.Entry) entry).getKey()).collect(Collectors.toList());
    }
    ...
    private void registerCacheEventListener() {
        final Object nativeCache = getCache().getNativeCache();
        if (nativeCache instanceof org.ehcache.Cache) {
            // CACHING: Listener configuration is completely different
            // ((org.ehcache.Cache) nativeCache).getCacheEventNotificationService().registerListener(this);
            log.debug("Registered user project cache as org.ehcache.Cache listener");
        } else {
    ...
    private final CacheEventListener<? extends String, ?> _cacheEventListener;
