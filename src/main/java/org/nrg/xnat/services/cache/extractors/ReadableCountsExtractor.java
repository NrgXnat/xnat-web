...
public class ReadableCountsExtractor extends AbstractGroupsAndPermissionsCacheDataExtractor<String, Map<String, Long>> {
    ...
    private static final String QUERY_USER_READABLE_SUBJECT_COUNT    = "SELECT greatest(SUM(subjs.COUNT), 0) AS ELEMENT_COUNT " +
                                                                       ...
                                                                       "         JOIN (SELECT project, COUNT(id) " +
                                                                       "               FROM xnat_subjectData " +
                                                                       "               GROUP BY project " +
                                                                       "               UNION " +
                                                                       "               SELECT project, COUNT(subject_id) " +
                                                                       "               FROM xnat_projectParticipant " +
                                                                       "               GROUP BY project) subjs ON xfm.field_value = subjs.project " +
                                                                       "WHERE gid.groups_groupid_xdat_user_xdat_user_id IN (:" + PARAM_USER_IDS + ") " +
                                                                       "   OR xea.xdat_user_xdat_user_id IN (:" + PARAM_USER_IDS + ")";
    private static final String QUERY_USER_READABLE_EXPERIMENT_COUNT = "SELECT xea.element_name, SUM(expts.SUM) AS ELEMENT_COUNT " +
                                                                       ...
                                                                       "               FROM (SELECT project, element_name, COUNT(id) AS COUNT " +
                                                                       ...
                                                                       "               GROUP BY project, element_name) expts ON xfm.field_value = expts.project AND xea.element_name = expts.element_name " +
                                                                       "WHERE gid.groups_groupid_xdat_user_xdat_user_id IN (:" + PARAM_USER_IDS + ") " +
    ...
    private Map<String, Long> getUserReadableSubjectsAndExperiments(final List<String> readableProjectIds, final List<Integer> userIds) {
        ...
        readableExperimentCounts.put(XnatSubjectdata.SCHEMA_ELEMENT_NAME, getTemplate().queryForObject(QUERY_USER_READABLE_SUBJECT_COUNT, userIdParameters, Long.class));
        readableExperimentCounts.putAll(getTemplate().query(QUERY_USER_READABLE_SCAN_COUNT, userIdParameters, ELEMENT_COUNT_EXTRACTOR));
