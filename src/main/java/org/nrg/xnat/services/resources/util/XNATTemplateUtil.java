...
public class XNATTemplateUtil {
	...
	private CatalogService _catalogService = null;
	    ...
	    public void setProjSubAssessExperScanAndReconData(String pId, String subId,  String assessId, String exptId,  String scanId,  String reconId) {
	        _catalogService = XDAT.getContextService().getBean(CatalogService.class);

	        final UserI user = getUser();
	        
	        setProjData(pId, user);
	        
	        setSubjData(subId,user);

	        setAssessData(assessId, user);

	      //type = StringUtils.defaultIfBlank((String) getParameter(request, "TYPE"), "out")
	        
	       setExperData(exptId, user);
	       
	       setScanData(scanId, user);
	       
	       setReconData(reconId, user);
	    }
	    
	    
	private void setProjData(String pId, UserI user) {
		if (pId != null) {
			proj = XnatProjectdata.getProjectByIDorAlias(pId, user, false);
		}
	}

	private void setSubjData(String subId, UserI user) {
		if (subId != null) {
			if (this.proj != null) {
	...
	private void setAssessData(String assessId, UserI user) {
		if (assessId != null) {
			for (String s : XftStringUtils.CommaDelimitedStringToArrayList(assessId)) {
				XnatExperimentdata assessed = XnatImagesessiondata.getXnatImagesessiondatasById(s, getUser(), false);
	...
	private void setExperData(String exptId, UserI user) {
		if (exptId != null) {
	...
	private void setScanData(String scanId, UserI user) {
	...
	private void setReconData(String reconId, UserI user) {
		if (reconId != null && assesseds.size() > 0) {
			CriteriaCollection cc = new CriteriaCollection("OR");

			for (XnatExperimentdata assessed : this.assesseds) {
				CriteriaCollection subcc = new CriteriaCollection("AND");
				subcc.addClause("xnat:reconstructedImageData/image_session_ID", assessed.getId());
			...
			recons = XnatReconstructedimagedata.getXnatReconstructedimagedatasByField(cc, user, completeDocument);
			if (recons.size() > 0) {
				if (type == null) {
					type = "out";
				}
...
//				experiment
	    ...
	    public String getComment() {
	        return "";
	        		//getQueryVariable(EventUtils.EVENT_COMMENT);
	    }
	    
	    public String getReason() {
	        return "";
	        		//getQueryVariable(EventUtils.EVENT_REASON);
	    }
	    ...
	    public XFTTable loadCatalogs(final List<String> resourceIds, final boolean includeURI, final boolean allowAll) throws Exception {
	        ...
	        return XFTTable.Execute(completedQuery, user.getDBName(), getUser().getUsername());
	    }

	    protected boolean isQueryVariableTrue(String string) {
