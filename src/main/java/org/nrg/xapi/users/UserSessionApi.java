...
public class UserSessionApi extends AbstractXapiProjectRestController {
	...
	public ResponseEntity<String> getUserSession(HttpSession httpSession,@ApiParam(value = "The ID of the csrf.") @PathVariable(required = false) final String csrf) throws Exception {
		log.debug("Controller Api- get User Session ");
		String session = _userSessionService.getJession(getSessionUser(),httpSession, csrf);
		if (session == null) {
