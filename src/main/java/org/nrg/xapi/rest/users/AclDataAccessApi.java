...
@Api("ACL Data Access API")
@XapiRestController
@RequestMapping(value = "/acl/access")
@Slf4j
public class AclDataAccessApi extends AbstractXapiRestController {
    ...
    @Autowired
    public AclDataAccessApi(final UserManagementServiceI userManagementService, final RoleHolder roleHolder, final GroupsAndPermissionsCache cache, final UserGroupServiceI groupService) {
        super(userManagementService, roleHolder);
        _cache = cache;
        _groupService = groupService;
    }

    @ApiOperation(value = "Gets the projects and roles associated with the current user.", response = Map.class, responseContainer = "List")
    ...
    @ResponseBody
    public Collection<Map<String, String>> getProjectRoles() {
        return getRoleHolder().getUserProjectRoles(getSessionUser());
    }
    ...
    @AuthDelegate(GuestUserAccessXapiAuthorization.class)
    @ResponseBody
    public List<String> getAvailableElementDisplays() {
        return AVAILABLE_ELEMENT_DISPLAYS;
    }
    ...
    public Date getLastModified() {
        return _cache.getUserLastUpdateTime(getSessionUser());
    }
    ...
    @ApiOperation(value = "Gets a list of the element displays of the specified type for the current user.",
                  ...
                  response = ElementDisplayModel.class, responseContainer = "List")
    ...
    @AuthDelegate(GuestUserAccessXapiAuthorization.class)
    @ResponseBody
    public List<ElementDisplayModel> getElementDisplays(@PathVariable final String display) throws DataFormatException {
        return getElementDisplayModels(getSessionUser(), display);
    }

    @ApiOperation(value = "Gets the projects and roles associated with the specified user. This can only be called by users with administrative privileges.", response = Map.class, responseContainer = "List")
    ...
    @ResponseBody
    public Collection<Map<String, String>> getUserProjectRoles(@ApiParam("The user to get the project roles for.") @PathVariable final String username) {
        return getRoleHolder().getUserProjectRoles(username);
    }
    ...
    @ResponseBody
    public Date getLastModifiedForUser(@ApiParam("The user to get the last modified timestamp for.") @PathVariable final String username) {
        return _cache.getUserLastUpdateTime(username);
    }
    ...
    @ResponseBody
    public List<ElementDisplayModel> getElementDisplaysForUser(@PathVariable final String username, @PathVariable final String display) throws DataFormatException, ServerException, NotFoundException {
        try {
            return getElementDisplayModels(Users.getUser(username), display);
    ...
    @ResponseBody
    public Map<String, String> getCacheStatus() throws NotFoundException {
        if (Initializing.class.isAssignableFrom(_cache.getClass())) {
            return ((Initializing) _cache).getInitializationStatus();
        }
        throw new NotFoundException("The located cache is not an initializing instance: " + _cache.getClass().getName());
    }
    ...
    @ResponseBody
    public void flushUserCacheStatus() {
    ...
    @ResponseBody
    public void flushUserCacheStatus(@ApiParam("Indicates the name of the user whose cache should be flushed. If not provided, this flushes the cache of the current user.") @PathVariable @Username final String username) throws InsufficientPrivilegesException {
    ...
    @ResponseBody
    public List<Integer> fixIrregularPermissions() {
        ...
        return _groupService.fixIrregularProjectGroups();
    }
