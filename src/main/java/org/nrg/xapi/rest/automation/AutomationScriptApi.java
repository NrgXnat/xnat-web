...
// TODO: Nothing is being done with the versionId parameter in updateScriptByScriptIdOrVersionId() and deleteScriptByScriptIdOrVersionId().
...
public class AutomationScriptApi extends AbstractXapiProjectRestController {
    ...
    @ApiOperation(value = "Gets the requested  automation script", notes = "Returns the  automation script with the specified ID", response = Script.class)
    ...
    public Script getScriptByScriptIdORVersionId(@ApiParam(value = "The ID of the script.") @PathVariable final String scriptId,
                                                 @ApiParam(value = "The ID of the version.") @PathVariable(required = false) final String versionId) throws NotFoundException, DataFormatException {
    ...
    @ApiOperation(value = "Gets all available automation scripts", notes = "Returns all automation scripts accessible to the current user", response = ScriptDto.class, responseContainer = "List")
    @ApiResponses({@ApiResponse(code = 200, message = "Returns the requested handlers."),
                   @ApiResponse(code = 400, message = "The requested scriptId wasn't found."),
                   @ApiResponse(code = 404, message = "The requested automation script wasn't found."),
                   @ApiResponse(code = 500, message = "An unexpected or unknown error occurred.")})
    ...
    public List<ScriptDto> getAllScripts() throws NotFoundException, InitializationException, InsufficientPrivilegesException, DataFormatException {
        log.debug("User {} requested a list of automation scripts", getSessionUser().getUsername());
        return _automationScriptService.findAll(getSessionUser());
    }

    @ApiOperation(value = "Update the specified automation script", notes = "Returns the  automation script with the specified ID")
    @ApiResponses({@ApiResponse(code = 200, message = "Returns the requested handlers."),
                   @ApiResponse(code = 400, message = "The requested scriptId wasn't found."),
                   @ApiResponse(code = 404, message = "The requested automation script wasn't found."),
                   @ApiResponse(code = 500, message = "An unexpected or unknown error occurred.")})
    ...
    @ApiOperation(value = "Delete the requested  automation script", notes = "Returns the  automation script with the specified ID")
    @ApiResponses({@ApiResponse(code = 200, message = "Returns the requested handlers."),
                   @ApiResponse(code = 400, message = "The requested scriptId wasn't found."),
                   @ApiResponse(code = 404, message = "The requested automation script wasn't found."),
                   @ApiResponse(code = 500, message = "An unexpected or unknown error occurred.")})
    ...
    @ApiOperation(value = "Gets the requested  automation script version", notes = "Returns the  automation script version with the specified ID", response = String.class, responseContainer = "List")
    @ApiResponses({@ApiResponse(code = 200, message = "Returns the requested handlers."),
                   @ApiResponse(code = 400, message = "The requested scriptId wasn't found."),
    ...
    public List<String> getScriptVersionByScriptId(@ApiParam(value = "The ID of the script.") @PathVariable final String scriptId) {
        log.debug("User {} requested automation script with ID {}", getSessionUser().getUsername(), scriptId);
