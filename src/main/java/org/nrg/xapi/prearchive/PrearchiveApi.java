...
public class PrearchiveApi extends AbstractXapiProjectRestController {
	...
	@XapiRequestMapping(value = "/prearchive", produces = MediaType.APPLICATION_JSON_VALUE, method = GET)
	public List<PrearchiveDto> getAllProjectConfigs(@ApiParam(value = "The ID of the project.") @PathVariable(required = false) final String projectId) throws NotFoundException, DataFormatException {
		log.debug("User {} requested configs", getSessionUser().getUsername());
		return _prearchiveService.findAllPrearchives(getSessionUser(), projectId);
	}
